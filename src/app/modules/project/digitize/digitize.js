(function() {
  ...
  function digitizeController(projects, projectResource, activities, digitalServices, $state, shortHistory, notificator) {
    ...
    console.log('services: ' + vm.project.services);
    for (var serviceKey in vm.project.services) {
      for (var tagKey in vm.project.services.tags) {
        console.log(vm.project.service[serviceKey].tags[tagKey].text);
      }
    }
   for (var activityKey in vm.project.activities) {
